...
The [Run tool window](https://www.jetbrains.com/help/idea/run-tool-window.html) is opened, and we see the results of the `clean` task, including all the Gradle output, so we can see what was run and what the results were. We can use this to help us debug problems if there are any issues.
