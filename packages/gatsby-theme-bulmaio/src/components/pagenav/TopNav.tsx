...
const TopNav: React.FC<TopNavProps> = (
  { parent, siblings, currentSlug, playlistLabel, kind = 'Tip' }
  ) => {
  ...
  return (
    <div style={{ marginBottom: '1em' }}>
      <div style={{ textAlign: 'center', marginBottom: '0.5em' }}>
        {parent && (
          <Link to={`${parent.slug}`} className="topnav-previous button is-size-7" style={{ border: 'none' }}
                title={parent.label}>
            <span className="icon">
      ...
      <div className="columns">
        ...
        <div className="column has-text-right is-one-quarter-desktop is-hidden-mobile">
          {next && (
            <Link to={`${next.slug}${playlistPrefix}`} className="topnav-previous button" style={{ border: 'none' }}
                  title={next.label}>
